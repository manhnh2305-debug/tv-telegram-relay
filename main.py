...
# ═════════════════════════════════════════════════════════════════════════════
# /signal — MAIN HANDLER
# v4.3.1: Crypto → Signal Bot + EA + channel NGAY (song song)
#          Forex/XAU → Signal Bot + EA → ack → channel (giữ nguyên v4.3.0)
# ═════════════════════════════════════════════════════════════════════════════
...
def signal_route():
    ...
    with lock:
        ...
        # ── v4.3.1: Crypto bypass zone (Pine không set zones cho crypto) ──
        ...
        # ── TẤT CẢ signals → pending + Signal Bot (giữ nguyên) ──
        ...
        # v4.3.1: copy data cho crypto channel (dùng ngoài lock)
        crypto_copy = dict(data) if is_crypto else None

    # ── Ngoài lock ────────────────────────────────────────────────────

    # 1) TẤT CẢ: gửi Signal Bot private (confirm → risk → EA)
    log.info(f"→ Signal Bot: {action} {symbol} @ {price} | SL:{sl} TP:{tp}")
    send_signal_message(data, callback_id)

    # 2) v4.3.1: CRYPTO THÊM → gửi channel NGAY, song song, không đợi EA
    if crypto_copy:
...
# ═════════════════════════════════════════════════════════════════════════════
# TELEGRAM WEBHOOK (giữ nguyên v4.3.0)
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with lock:
        if order_id in mt5_queue:
            ...
            if status == "filled":
                ...
                # ── v4.3.1: Channel routing sau EA ack ──
                # Crypto: KHÔNG route lại (đã gửi ngay ở /signal)
                # Forex/XAU: route đến channel tương ứng (giữ nguyên v4.3.0)
